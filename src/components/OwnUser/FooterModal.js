...
import SmileSvg from "../MakePost/SmileSvg";
...
const FooterModal = ({ post, setPost }) => {
  ...
  const [toggleLikedMana, setToggleLiked] = useState(post.userLikedPhoto);
  ...
  const handleToggleLiked = async () => {
    ...
    await firebase
      ...
      .doc(post.docId)
      .update({
        likes: toggleLikedMana
          ? FieldValue.arrayRemove(user.uid)
          : FieldValue.arrayUnion(user.uid),
      });

    setLikes((likes) => (toggleLikedMana ? likes - 1 : likes + 1));
  };
    ...
    <>
        ...
        <div className="flex ml-2 mt-1">
          <div>
            <svg
              ...
              stroke="currentColor"
              tabIndex={0}
              className={`w-8 mr-4 outline-none select-none cursor-pointer ${
                toggleLikedMana ? "fill-red-500 text-red-500" : "text-black"
              }`}
            >
              <path
                strokeLinecap="round"
          ...
          <div>
            <svg
              ...
              strokeWidth={2}
              stroke="currentColor"
              className="w-8 outline-none text-black-light select-none cursor-pointer"
            >
              <path
                strokeLinecap="round"
                strokeLinejoin="round"
      ...
      <div className="flex items-center border justify-between relative">
        <div className="pl-1" onClick={() => setEmoji((prev) => !prev)}>
          <BiSmile className="text-gray-400 text-2xl" />
        </div>
        {emoji && (
          <div
        ...
        <div>
          <button
            className="text-blue-500 text-sm font-semibold text-sm rounded-xl text-blue w-14 h-8"
            onClick={addComment}
          >
            Post
          </button>
